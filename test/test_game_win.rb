class TestGameWin < MiniTest::Test
  ...
  def test_supply_win
    g = Game.create_new([HouseStark.create_new, HouseLannister.create_new, HouseBaratheon.create_new])
    g.map = Map.create_new([])
    g.map.area(Winterfell).receive_token!(Footman.create_new(HouseStark)) # 1 VP, 1 stronghold, 1 supply
    g.map.area(Lannisport).receive_token!(Footman.create_new(HouseLannister)) # 1 VP, 1 stronghold, 2 supply
    g.map.area(Dragonstone).receive_token!(Footman.create_new(HouseBaratheon)) # 1 VP, 1 stronghold, 1 supply
  ...
  def test_power_win
    ...
    g.map = Map.create_new([])
    g.map.area(Winterfell).receive_token!(Footman.create_new(HouseStark)) # 1 VP, 1 stronghold, 1 supply
    g.map.area(TheStoneyShore).receive_token!(Footman.create_new(HouseStark)) # 1 supply
    g.map.area(Lannisport).receive_token!(Footman.create_new(HouseLannister)) # 1 VP, 1 stronghold, 2 supply
    g.map.area(Dragonstone).receive_token!(Footman.create_new(HouseBaratheon)) # 1 VP, 1 stronghold, 1 supply
    g.map.area(Kingswood).receive_token!(Footman.create_new(HouseBaratheon)) # 1 supply
